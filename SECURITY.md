...
- **Urmă radio locală, cu Bluetooth pornit (v1.5.0)** — cât timp transportul Bluetooth e activ, telefonul **se anunță continuu** cu un identificator **stabil**, derivat din DID-ul tău (`did8` = primii 8 octeți din SHA-256(DID)). Nu dezvăluie DID-ul și nu expune conținut, dar oricine e **fizic în apropiere** cu un scanner BLE poate **corela prezența aceluiași telefon în timp și spațiu** (ex.: „dispozitivul X trece pe aici zilnic la 8:00"). E o trasabilitate *locală*, nu una de rețea — dar e reală. **Rotația identificatorului** (HMAC pe fereastră de timp) e pe roadmap. Până atunci: dacă te temi de urmărire fizică, **ține transportul Bluetooth oprit** (e oprit implicit).
- **Bluetooth ≠ mesh** — livrarea prin Bluetooth e **directă, în proximitate**: nu există multi-hop și nici store-and-forward prin telefoanele altora. Nu te baza pe el ca pe o rețea rezistentă la cenzură; e o scurtătură când destinatarul e lângă tine.
